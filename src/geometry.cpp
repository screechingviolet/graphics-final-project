...
void Realtime::setupPrimitives(VboVao* shape_ids, const std::vector<GLfloat>& triangles, bool anim) {
    ...
    if (anim) {
        ...
        glVertexAttribIPointer(2, 4, GL_INT, 14 * sizeof(GLint), reinterpret_cast<void*>(6 * sizeof(GLfloat)));
        glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, 14 * sizeof(GLfloat), reinterpret_cast<void*>(6 * sizeof(GLfloat) + 4 * sizeof(GLint)));
    } else {
